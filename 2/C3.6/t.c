...
{
  int scode; 
  //  oldISR = isr = (int *)*((int *)(VIC_BASE_ADDR+0x30)); // read ISR address in vectorAddr      
  status = *((int *)(VIC_BASE_ADDR)); // read status register     
  //  kprintf("status=%x\n", status);
...
int main()
{
   ...
   *(kp->base+KCNTL) = 0x12; // 1<<4;
   ...
   kprintf("Program C3.6 start: test Nested Interrupts\n");
